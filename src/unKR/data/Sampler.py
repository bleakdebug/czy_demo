...
class RandomSampler:
    def __init__(self, args):
        ...
        Args:
            args: Configuration object containing num_ent
        """
        # 支持从ArgumentParser或字典对象获取num_ent属性
        if hasattr(args, 'num_ent'):
            self.num_ent = args.num_ent  # 从ArgumentParser对象获取
        elif isinstance(args, dict) and 'num_ent' in args:
            self.num_ent = args['num_ent']  # 从字典获取
        else:
            # 如果参数既不是ArgumentParser对象也不是字典，则假设它本身就是num_ent值
            self.num_ent = args
        
        # 确保num_ent是整数
        try:
            self.num_ent = int(self.num_ent)
        except (ValueError, TypeError):
            print(f"警告：num_ent '{self.num_ent}'不是有效整数，使用默认值15000")
            self.num_ent = 15000
            
        print(f"RandomSampler初始化: num_ent = {self.num_ent}")
    ...
    def sample(self, batch_size):
        ...
        Returns:
            Tensor of random entity indices
        """
        return torch.randint(0, self.num_ent, (batch_size,))
        
    def __call__(self, batch_size):
        """Callable interface for sampling.
